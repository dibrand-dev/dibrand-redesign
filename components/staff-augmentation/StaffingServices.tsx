                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-y-12 gap-x-8 lg:gap-x-16">
                    {categories.map((category, idx) => (
                        <div key={idx} className="flex flex-col">
                            <h3 className="text-xl font-bold text-[#D83484] mb-8 font-outfit tracking-tight uppercase">
                                {category.title}
                            </h3>
                            <div className="space-y-8">
                                {category.roles.map((role, roleIdx) => (
                                    <div key={roleIdx} className="flex items-start gap-3 group">
                                        <div className="flex-shrink-0 w-6 h-6 rounded-md bg-[#D83484] flex items-center justify-center text-white mt-1 group-hover:scale-110 transition-transform">
