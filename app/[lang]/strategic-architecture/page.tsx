...
import StrategicArchitectureHero from "@/components/strategic-architecture/StrategicArchitectureHero";
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    return {
        title: isEs ? "Strategic Architecture | Dibrand" : "Strategic Architecture | Dibrand",
        description: isEs 
            ? "Diseñamos bases técnicas sólidas y escalables que soportan el crecimiento acelerado de tu plataforma."
            : "We design solid, scalable technical foundations that support the accelerated growth of your platform.",
        openGraph: {
            title: "Strategic Architecture | Dibrand",
            description: isEs 
                ? "Planos arquitectónicos de software de nivel empresarial para alto crecimiento."
                : "Enterprise-grade software blueprints for high growth.",
        }
    };
}

export default async function StrategicArchitecturePage(props: { params: Promise<{ lang: "en" | "es" }> }) {
    const params = await props.params;
    const dict = await getDictionary(params.lang);
    const content = dict.strategicArchitecture;
    ...
    const pillarIcons = [Layers, CloudCog, Activity, Lock];
