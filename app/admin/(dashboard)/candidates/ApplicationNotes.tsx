...
function NoteMenu({
    ...
    return (
        <div ref={ref} className="relative">
            ...
            {open && (
                <div className="absolute right-0 top-8 z-50 w-40 bg-admin-card-bg border border-admin-border rounded-xl shadow-lg py-1 text-sm animate-in fade-in slide-in-from-top-1 duration-100">
                    <button
                        onClick={handleEdit}
                        className="flex items-center gap-2.5 w-full px-3 py-2 text-admin-text-primary hover:bg-admin-bg/50 transition-colors font-medium"
                    >
                    ...
                    <button
                        ...
                        className="flex items-center gap-2.5 w-full px-3 py-2 text-red-500 hover:bg-red-500/10 transition-colors font-medium"
                    >
                        <Trash2 size={13} />
                        Remove
                    </button>
...
function NoteItem({
    ...
    return (
        <div className="flex gap-3 group">
            {/* Avatar */}
            <div className="w-9 h-9 rounded-full overflow-hidden bg-admin-bg flex items-center justify-center shrink-0 border border-admin-border">
            ...
            <div className="flex-1 min-w-0 space-y-1">
                ...
                <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                        <span className="text-sm font-bold text-admin-text-primary truncate">
                            {note.author_name}
                        </span>
                ...
                {editing ? (
                    <div className="space-y-2">
                        <textarea
                            ...
                            autoFocus
                            rows={3}
                            className="w-full p-3 text-sm bg-admin-bg/50 border border-admin-accent/40 rounded-xl focus:ring-2 focus:ring-admin-accent/20 focus:border-admin-accent outline-none text-admin-text-primary font-medium italic resize-none transition-all"
                        />
                        <div className="flex items-center gap-2 justify-end">
                            ...
                            <button
                                ...
                                className="flex items-center gap-1.5 px-4 py-1.5 text-xs font-bold text-white bg-admin-accent rounded-lg hover:opacity-90 disabled:opacity-40 transition-all uppercase tracking-widest"
                            >
                                <Check size={12} />
                                {saving ? 'Saving...' : 'Save'}
                            </button>
                    ...
                    <div className="bg-admin-bg/50 p-3 rounded-2xl rounded-tl-none border border-transparent group-hover:border-admin-border transition-all">
                        <p className="text-sm text-admin-text-secondary leading-relaxed whitespace-pre-wrap font-medium italic">
                            {note.note_text}
                        </p>
                    </div>
                )}
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="space-y-6 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                ...
                {notes.length === 0 && (
                    <div className="text-center py-12 bg-admin-bg/30 rounded-2xl border border-dashed border-admin-border text-admin-text-secondary text-sm italic font-medium">
